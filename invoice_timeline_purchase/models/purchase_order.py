...
class PurchaseOrder(models.Model):
    ...
    invoice_timeline_template_id = Many2one('account.timeline.template', string=_('Invoice timeline'),
                                            ondelete='restrict', copy=False, domain=[
    ...
    def create_invoices_from_timeline(self):
        ...
        for purchase_id in self:
            lines = [line_id._get_data_line_invoice_timeline() for line_id in purchase_id.order_line]
            ...
            invoice_ids = purchase_id.invoice_timeline_template_id.create_invoices(
                purchase_id.partner_id, lines, invoice_type='in_invoice', **default_value)
            purchase_id.invoice_ids = [(4, invoice_id.id) for invoice_id in invoice_ids]
            purchase_id.invoice_timeline_template_id = purchase_id.invoice_ids.mapped('account_timeline_template_id')

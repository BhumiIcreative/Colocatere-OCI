...
class AccountMoveDateUpdate(models.TransientModel):
    ...
    account_timeline_template_id = fields.Many2one(
        "account.timeline.template", string="Timeline", readonly=True
    )

    line_ids = fields.One2many(
        "account.move.date.update.line", "wizard_id", string="Lines"
    )
    ...
    def confirm(self):
        ...
        for line_id in self.line_ids.filtered(lambda l: l.writable):
            ...
            for invoice_line_id in line_id.invoice_id.line_ids.filtered(
    ...
    def _compute_date(self, base_line_id):
        ...
        lines_per_date = Script.groupby(
            self.account_timeline_template_id.line_ids, "sequence"
        )
        expected_invoice_count = len(lines_per_date.keys())
        if invoice_count != expected_invoice_count:
            raise UserError(
                _(
                    "I should have %s invoices but I have %s, so I can't compute next date by myself."
                )
        ...
        for line_id in self.line_ids:
            if line_id == base_line_id:
                break
            i += 1
        dates = self.account_timeline_template_id.line_ids[i]._get_date(
            base_line_id.new_invoice_date_due
        )
        line_ids = self.line_ids.filtered(
            lambda l: l.invoice_date_due >= base_line_id.invoice_date_due
        )

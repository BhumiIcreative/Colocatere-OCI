...
class AccountTimelineTemplate(models.Model):
    ...
    invoice_count = fields.Integer(_('Invoice count'), compute='_cpt_invoice_count', store=True)
    name = fields.Char(_('Name'), required=True)
    sequence = fields.Integer(_('Sequence'))
    used_on = fields.Char(_('Used on'), readonly=True)
    ...
    def _group_lines_by_categ_id(self, lines):
        ...
        for line in lines:
            ...
            key = categ_id if categ_id in to_exclude_product_categ_ids else ProductCategory
    ...
    def create_invoices(self, partner_id, lines, date_start=False, invoice_type='out_invoice',
        ...
        invoiceable_lines_by_date = template_copy_id._get_invoiceable_lines_by_date(prices, date_start)
        move_ids = AccountMove
        for date, lines in invoiceable_lines_by_date.items():
            move_id = AccountMove.create(dict({
        ...
        template_copy_id.name = '%s - %s: %s (%s to %s)' % (
            template_copy_id.name, partner_id.name, amount_total, min_date, max_date)

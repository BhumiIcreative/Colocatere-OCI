...
class AccountTimelineTemplateLine(models.Model):
    ...
    percent = fields.Float("Percent", required=True, default=100)
    sequence = fields.Integer("Sequence", compute="_cpt_sequence", store=True)
    ...
    def _check_percent_constraints(self):
        ...
        for record in self:
            if record.percent < 0:
                raise UserError(
                    _("Percent must be positive! (Currently: %s)") % record.percent
                )
            elif record.percent > 100:
                raise UserError(
                    _("Percent must be less than or equal to 100! (Currently: %s)")
                    % record.percent
                )
